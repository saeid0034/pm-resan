...
def welcome(m):
    cid = m.chat.id
    markup = types.InlineKeyboardMarkup()
    b = types.InlineKeyboardButton("music searcher",callback_data='music')
    markup.add(b)
    ret_msg = bot.send_message(cid, "UNF Help", disable_notification=True, reply_markup=markup)
    assert ret_msg.message_id
...
def inline(query):
    ...
    if query.query.split()[0] == 'bold':
        text = query.query.replace('bold','')
        bold = types.InlineQueryResultArticle('1','Text ='+text, types.InputTextMessageContent('<b>'+text+'</b>',parse_mode='HTML'))
    if query.query.split()[0] == 'italic':
        text = query.query.replace('italic','')
        italic = types.InlineQueryResultArticle('1','Text ='+text, types.InputTextMessageContent('<i>'+text+'</i>',parse_mode='HTML'))
    if query.query.split()[0] == 'code':
        text = query.query.replace('italic','')
        code = = types.InlineQueryResultArticle('1','Text ='+text, types.InputTextMessageContent('<code>'+text+'</code>',parse_mode='HTML'))
		...
		if m.new_chat_participant.id == bot.get_me().id:
			...
			if str(cid) not in chat:
				...
				with open('chat.txt', 'a') as f:
					f.write(str(cid)+"\n")
			bot.send_message(cid, "*Hi all \n special tnx to invite me \n i am UNF bot a free fun bot :D*", parse_mode="Markdown")
